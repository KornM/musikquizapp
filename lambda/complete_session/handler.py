"""
Complete Session Lambda Handler

This Lambda function marks a session as completed/inactive.
Participants can no longer submit answers to completed sessions.

Endpoint: POST /admin/quiz-sessions/{sessionId}/complete
"""
...
def lambda_handler(event, context):
    """
    Handle complete session requests.
    ...
    Returns:
        Success (200): Session marked as completed
    ...
    try:
        ...
        try:
            update_item(
                ...
                "SET #status = :status, completedAt = :completedAt, currentRound = :null",
                {
                    ":status": "completed",
            ...
            return error_response(500, "DATABASE_ERROR", "Failed to complete session")
        ...
        print(f"Unexpected error in complete session: {str(e)}")
        import traceback
