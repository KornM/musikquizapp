"""
Upload Image Lambda Handler

This Lambda function handles uploading image files to S3 for quiz rounds.
It validates the admin JWT token, generates a unique S3 key, and uploads the image data.

Endpoint: POST /admin/image
"""
import json
import os
import sys
import uuid
import base64
from datetime import datetime

# Add common utilities to path
sys.path.insert(0, os.path.join(os.path.dirname(__file__), "..", "common"))

from auth import validate_token
from cors import add_cors_headers
from errors import error_response
import jwt
import boto3
from botocore.exceptions import ClientError


# Environment variables
AUDIO_BUCKET = os.environ.get("AUDIO_BUCKET", "music-quiz-audio")

# Initialize S3 client
s3_client = boto3.client("s3")


def lambda_handler(event, context):
    """
    Handle image upload requests.

    Expected headers:
        Authorization: Bearer <jwt_token>
        Content-Type: image/* or application/json (for base64)

    Expected input (JSON with base64):
        {
            "imageData": "base64_encoded_image",
            "fileName": "picture.jpg",
            "sessionId": "uuid"
        }

    Or raw binary data in body with query parameters

    Returns:
        Success (201):
            {
                "imageKey": "sessions/{sessionId}/images/{uuid}.jpg",
                "url": "presigned_url"
            }

        Error (400): Invalid request
        Error (401): Missing or invalid token
        Error (403): Insufficient permissions
        Error (500): Internal server error
    """
    try:
        # Validate Authorization header
        headers = event.get("headers", {})
        auth_header = headers.get("Authorization") or headers.get("authorization")

        if not auth_header:
            return error_response(
                401, "MISSING_TOKEN", "Authorization header is required"
            )

        # Extract token from "Bearer <token>" format
        if not auth_header.startswith("Bearer "):
            return error_response(
                401,
                "INVALID_AUTH_FORMAT",
                "Authorization header must be 'Bearer <token>'",
            )

        token = auth_header[7:]  # Remove "Bearer " prefix

        # Validate JWT token
        try:
            payload = validate_token(token)
        except jwt.ExpiredSignatureError:
            return error_response(401, "TOKEN_EXPIRED", "Token has expired")
        except jwt.InvalidTokenError:
            return error_response(401, "INVALID_TOKEN", "Invalid token")

        # Check if user has admin role
        if payload.get("role") != "admin":
            return error_response(
                403, "INSUFFICIENT_PERMISSIONS", "Admin role required"
            )

        # Parse request body
        if not event.get("body"):
            return error_response(400, "INVALID_REQUEST", "Request body is required")

        content_type = headers.get("Content-Type") or headers.get("content-type", "")

        # Handle JSON with base64 encoded audio
        if "application/json" in content_type:
            try:
                body = json.loads(event["body"])
            except json.JSONDecodeError:
                return error_response(
                    400, "INVALID_JSON", "Request body must be valid JSON"
                )

            image_data_b64 = body.get("imageData")
            file_name = body.get("fileName", "image.jpg")
            session_id = body.get("sessionId")

            if not image_data_b64:
                return error_response(
                    400,
                    "MISSING_FIELDS",
                    "imageData is required",
                    {"required_fields": ["imageData"]},
                )

            if not session_id:
                return error_response(
                    400,
                    "MISSING_FIELDS",
                    "sessionId is required",
                    {"required_fields": ["sessionId"]},
                )

            # Decode base64 image data
            try:
                image_data = base64.b64decode(image_data_b64)
            except Exception as e:
                print(f"Base64 decode error: {str(e)}")
                return error_response(
                    400, "INVALID_IMAGE_DATA", "imageData must be valid base64"
                )

        else:
            # Handle raw binary data
            image_data = event["body"]
            if event.get("isBase64Encoded"):
                image_data = base64.b64decode(image_data)
            else:
                image_data = (
                    image_data.encode("utf-8")
                    if isinstance(image_data, str)
                    else image_data
                )

            # Get session ID from query parameters
            query_params = event.get("queryStringParameters") or {}
            session_id = query_params.get("sessionId")
            file_name = query_params.get("fileName", "image.jpg")

            if not session_id:
                return error_response(
                    400,
                    "MISSING_SESSION_ID",
                    "sessionId is required in query parameters or JSON body",
                )

        # Extract file extension
        file_extension = file_name.split(".")[-1] if "." in file_name else "jpg"

        # Validate image extension
        valid_extensions = ["jpg", "jpeg", "png", "gif", "webp"]
        if file_extension.lower() not in valid_extensions:
            return error_response(
                400,
                "INVALID_FILE_TYPE",
                f"File extension must be one of: {', '.join(valid_extensions)}",
                {"provided": file_extension, "valid": valid_extensions},
            )

        # Generate unique S3 key
        unique_id = str(uuid.uuid4())
        image_key = f"sessions/{session_id}/images/{unique_id}.{file_extension}"

        # Determine content type
        content_type_map = {
            "jpg": "image/jpeg",
            "jpeg": "image/jpeg",
            "png": "image/png",
            "gif": "image/gif",
            "webp": "image/webp",
        }
        content_type = content_type_map.get(file_extension.lower(), "image/jpeg")

        # Upload to S3
        try:
            s3_client.put_object(
                Bucket=AUDIO_BUCKET,
                Key=image_key,
                Body=image_data,
                ContentType=content_type,
            )
        except ClientError as e:
            print(f"S3 upload error: {str(e)}")
            return error_response(500, "UPLOAD_ERROR", "Failed to upload image file")

        # Generate presigned URL (valid for 1 hour)
        try:
            presigned_url = s3_client.generate_presigned_url(
                "get_object",
                Params={"Bucket": AUDIO_BUCKET, "Key": image_key},
                ExpiresIn=3600,
            )
        except ClientError as e:
            print(f"Presigned URL generation error: {str(e)}")
            # File uploaded but URL generation failed - still return success
            presigned_url = None

        # Return success response
        response_body = {"imageKey": image_key}
        if presigned_url:
            response_body["url"] = presigned_url

        response = {
            "statusCode": 201,
            "body": json.dumps(response_body),
        }

        return add_cors_headers(response)

    except Exception as e:
        # Log unexpected errors
        print(f"Unexpected error in upload image: {str(e)}")
        import traceback

        traceback.print_exc()

        return error_response(500, "INTERNAL_ERROR", "An unexpected error occurred")
