...
This Lambda function generates CloudFront URLs for audio files.
...
def lambda_handler(event, context):
    ...
    """
    try:
        # Extract audio key from query parameters
        query_params = event.get("queryStringParameters") or {}
        audio_key = query_params.get("key")

        if not audio_key:
            return error_response(
                400,
                "MISSING_AUDIO_KEY",
                "Audio key is required as query parameter 'key'",
            )
        ...
        # Generate CloudFront URL if domain is configured, otherwise use S3 presigned URL
        if CLOUDFRONT_DOMAIN:
            # Use CloudFront URL (no signing needed as CloudFront has OAI access)
            # Audio files are stored as: sessions/{sessionId}/audio/{uuid}.mp3
